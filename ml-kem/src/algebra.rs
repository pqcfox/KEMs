...
use core::ops::Mul;
pub use module_lattice::algebra::Field;
use module_lattice::encode::{ArraySize, Encode};
pub use module_lattice::util::Truncate;
use sha3::digest::XofReader;

use module_lattice::algebra;
use module_lattice::define_field;
...
pub struct MlKemNttPolynomial(NttPolynomial);
pub struct MlKemNttVector<K: ArraySize>(NttVector<K>);

define_field!(BaseField, u16, u32, u64, 3329);

pub trait BaseCaseMultiply {
    fn base_case_multiply(a0: Elem, a1: Elem, b0: Elem, b1: Elem, i: usize) -> (Elem, Elem);
}

impl BaseCaseMultiply for Elem {
    ...
    fn base_case_multiply(a0: Elem, a1: Elem, b0: Elem, b1: Elem, i: usize) -> (Elem, Elem) {
        let a0: u32 = a0.0.into();
        let a1: u32 = a1.0.into();
        let b0: u32 = b0.0.into();
        let b1: u32 = b1.0.into();
        let g = u32::from(GAMMA[i].0);

        let b1g = BaseField::barrett_reduce(b1 * g);

        let c0 = BaseField::barrett_reduce(a0 * b0 + a1 * u32::from(b1g));
...
impl<K: ArraySize> SampleCbd for Vector<K> {
        ...
        Eta: CbdSamplingSize,
    {
        Vector::new(Array::from_fn(|i| {
            ...
            Polynomial::sample_cbd::<Eta, K>(&prf_output)
        }))
    }
}
...
impl Mul<&MlKemNttPolynomial> for &MlKemNttPolynomial {
    type Output = MlKemNttPolynomial;

    fn mul(self, rhs: &MlKemNttPolynomial) -> MlKemNttPolynomial {
        let mut out = NttPolynomial::new(Array::default());

        for i in 0..128 {
            let self_inner = &self.0;
            let rhs_inner = &rhs.0;
            let (c0, c1) = Elem::base_case_multiply(
                self_inner.0[2 * i],
                self_inner.0[2 * i + 1],
                rhs_inner.0[2 * i],
                rhs_inner.0[2 * i + 1],
        ...
        MlKemNttPolynomial(out)
    }
}

impl<K: ArraySize> Mul<&MlKemNttVector<K>> for &MlKemNttVector<K> {
    type Output = MlKemNttVector<K>;

    fn mul(self, rhs: &MlKemNttVector<K>) -> MlKemNttVector<K> {
        let self_inner = self.0;
        let rhs_inner = self.0;

        self_inner
            .0
            .iter()
            .zip(rhs_inner.0.iter())
            .map(|(x, y)| (MlKemNttPolynomial(x) * MlKemNttPolynomial(y)).0)
